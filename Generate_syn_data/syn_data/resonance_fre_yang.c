...
void resonance_fre_generator(char strA[], char strB[], char strC[], char strD[], char strA1[], char strB1[], char strD1[], float radius[]){
    long i,j,r_index;
    FILE *A,*B,*C,*D,*A1,*B1,*C1,*D1;
    static float as,Pa,Ph,Ps,P0,tidalheight,times[time_loop+1],X_value,frequency[freq_loop+1],X_ast,Y_ast,d_ast,f_ast,A_value, B_value,Xm = 0.0,Ym =0.0,ans,final_ans,k,f0,dr,dt,d,df,alpha,density,Sg,P0ave,r;
    static int depth = 10;
  ...
  for (i=1; i<=time_loop; i++)
    {
        ...
        for (j=1; j<=freq_loop; j++)
        {
            ...
            for (r_index=0; r_index < 2; r_index++)
            {
                r = radius[r_index];
...
int main(int argc, char *argv[])
{
    ...
    FILE *radius;
    float r[2] = {atof(argv[1])*1e-2,atof(argv[2])*1e-2};
    ...
    fprintf(radius,"%.2f_%.2f\n",r[0]*1e3,r[1]*1e3);
    fclose(radius);
    sprintf(strA,"%.3f_%.3f",r[0]*1e3,r[1]*1e3);
    sprintf(strB,"%.3f_%.3f",r[0]*1e3,r[1]*1e3);    
    sprintf(strC,"%.3f_%.3f",r[0]*1e3,r[1]*1e3);
    sprintf(strD,"%.3f_%.3f",r[0]*1e3,r[1]*1e3);
    sprintf(strE,"%.3f_%.3f",r[0]*1e3,r[1]*1e3);
    sprintf(strF,"%.3f_%.3f",r[0]*1e3,r[1]*1e3);
    sprintf(strG,"%.3f_%.3f",r[0]*1e3,r[1]*1e3);
    sprintf(strH,"%.3f_%.3f",r[0]*1e3,r[1]*1e3);
    sprintf(strA1,"%.3f_%.3f",r[0]*1e3,r[1]*1e3);
    sprintf(strB1,"%.3f_%.3f",r[0]*1e3,r[1]*1e3);
    sprintf(strD1,"%.3f_%.3f",r[0]*1e3,r[1]*1e3);
    sprintf(strv,"%.3f_%.3f",r[0]*1e3,r[1]*1e3);
